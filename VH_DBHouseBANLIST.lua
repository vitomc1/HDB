...
local slot12_a5323 = {}
local slot14_a2516 = imgui.ImBool(false)
local slot15_a2515 = imgui.ImBool(false)

local CheckingHouse = {} -- // пїЅпїЅпїЅ /chksign !!!!!
...
function main()
	...
	getbanlist(false)
	sampAddChatMessage("{FF7F50}[HDB] {ffffff}База Данных домов игроков. /hdb - основное меню", 0x0ff8f50)
	sampRegisterChatCommand("hbans", function()
		getbanlist(true)
	end)

	sampRegisterChatCommand("checkbans", function(arg)
		local dataDay =	os.date("%d",os.time()+432000)
		local dataMon =	os.date("%m",os.time()+4320000)
		local dataYear = os.date("20%y",os.time()+432000)
		if arg ~= nil and #arg > 0 then
			if arg == "info" then
				cleanBanlist()
				for cleanbl = #db["HOUSE"], 1, -1 do
					for key, value in pairs(bans) do
						timeDay1 = os.date("%d",os.time()-1382400) -- // 16 day nazad
						timeMonth1 = os.date("%m",os.time()-1382400)
						timeYear1 = os.date("20%y",os.time()-1382400)
						if bans[key].date.day == os.date("%d",os.time()-1814400) and bans[key].date.month == os.date("%m",os.time()-1814400) and bans[key].date.year == os.date("20%y",os.time()-1814400) then
							if db["HOUSE"][cleanbl]["lastowner"]:lower() == value.name:lower() and not checkUnBan(value.name, value.date) then
								sampAddChatMessage("Сегодня{FFFACD} Дом {51FF61}№"..db["HOUSE"][cleanbl]["num"]..".{FFFACD} Парк: {51FF61}"..db["HOUSE"][cleanbl]["park"]..".{FFFACD} Владелец {51FF61}"..bans[key].name..".{FFFACD} Район: {51FF61}"..db["HOUSE"][cleanbl]["area"]..".", 0x0ff0000)
							end
						end
						if bans[key].date.day == os.date("%d",os.time()-1728000) and bans[key].date.month == os.date("%m",os.time()-1728000) and bans[key].date.year == os.date("20%y",os.time()-1728000) then
							if db["HOUSE"][cleanbl]["lastowner"]:lower() == value.name:lower() and not checkUnBan(value.name, value.date) then
								sampAddChatMessage("Завтра{FFFACD} Дом {51FF61}№"..db["HOUSE"][cleanbl]["num"]..".{FFFACD} Парк: {51FF61}"..db["HOUSE"][cleanbl]["park"]..".{FFFACD} Владелец {51FF61}"..bans[key].name..".{FFFACD} Район: {51FF61}"..db["HOUSE"][cleanbl]["area"]..".", 0x0ff0000)
							end
						end
						if bans[key].date.day == os.date("%d",os.time()-1641600) and bans[key].date.month == os.date("%m",os.time()-1641600) and bans[key].date.year == os.date("20%y",os.time()-1641600) then
							if db["HOUSE"][cleanbl]["lastowner"]:lower() == value.name:lower() and not checkUnBan(value.name, value.date) then
								sampAddChatMessage("2дн{FFFACD} Дом {51FF61}№"..db["HOUSE"][cleanbl]["num"]..".{FFFACD} Парк: {51FF61}"..db["HOUSE"][cleanbl]["park"]..".{FFFACD} Владелец {51FF61}"..bans[key].name..".{FFFACD} Район: {51FF61}"..db["HOUSE"][cleanbl]["area"]..".", 0x0ff0000)
							end
						end
						if bans[key].date.day == os.date("%d",os.time()-1555200) and bans[key].date.month == os.date("%m",os.time()-1555200) and bans[key].date.year == os.date("20%y",os.time()-1555200) then
							if db["HOUSE"][cleanbl]["lastowner"]:lower() == value.name:lower() and not checkUnBan(value.name, value.date) then
								sampAddChatMessage("3дн{FFFACD} Дом {51FF61}№"..db["HOUSE"][cleanbl]["num"]..".{FFFACD} Парк: {51FF61}"..db["HOUSE"][cleanbl]["park"]..".{FFFACD} Владелец {51FF61}"..bans[key].name..".{FFFACD} Район: {51FF61}"..db["HOUSE"][cleanbl]["area"]..".", 0x0ff0000)
							end
						end
						if bans[key].date.day == os.date("%d",os.time()-1468800) and bans[key].date.month == os.date("%m",os.time()-1468800) and bans[key].date.year == os.date("20%y",os.time()-1468800) then
							if db["HOUSE"][cleanbl]["lastowner"]:lower() == value.name:lower() and not checkUnBan(value.name, value.date) then
								sampAddChatMessage("4дн{FFFACD} Дом {51FF61}№"..db["HOUSE"][cleanbl]["num"]..".{FFFACD} Парк: {51FF61}"..db["HOUSE"][cleanbl]["park"]..".{FFFACD} Владелец {51FF61}"..bans[key].name..".{FFFACD} Район: {51FF61}"..db["HOUSE"][cleanbl]["area"]..".", 0x0ff0000)
							end
						end
						if bans[key].date.day == os.date("%d",os.time()-1382400) and bans[key].date.month == os.date("%m",os.time()-1382400) and bans[key].date.year == os.date("20%y",os.time()-1382400) then
							if db["HOUSE"][cleanbl]["lastowner"]:lower() == value.name:lower() and not checkUnBan(value.name, value.date) then
								sampAddChatMessage("5дн{FFFACD} Дом {51FF61}№"..db["HOUSE"][cleanbl]["num"]..".{FFFACD} Парк: {51FF61}"..db["HOUSE"][cleanbl]["park"]..".{FFFACD} Владелец {51FF61}"..bans[key].name..".{FFFACD} Район: {51FF61}"..db["HOUSE"][cleanbl]["area"]..".", 0x0ff0000)
							end
						end
				...
				--[[		if bans[key].date.day <= timeDay1 and bans[key].date.month <= timeMonth1 and bans[key].date.year <= timeYear1 then
							if db["HOUSE"][cleanbl]["lastowner"]:lower() == value.name:lower() and not checkUnBan(value.name, value.date) then
								sampAddChatMessage("-{FFFACD} ["..bans[key].date.day..":"..bans[key].date.month..":"..bans[key].date.year.."]  Дом {51FF61}№"..db["HOUSE"][cleanbl]["num"]..".{FFFACD} Парк: {51FF61}"..db["HOUSE"][cleanbl]["park"]..".{FFFACD} Владелец {51FF61}"..bans[key].name..".{FFFACD} Район: {51FF61}"..db["HOUSE"][cleanbl]["area"]..".", 0x0ff0000)
			...
			for cleanbl = #db["HOUSE"], 1, -1 do
				for key, value in pairs(bans) do
					...
					if bans[key].date.day == timeDay and bans[key].date.month == timeMonth and bans[key].date.year == timeYear then
						if db["HOUSE"][cleanbl]["lastowner"]:lower() == value.name:lower() and not checkUnBan(value.name, value.date) then
							sampAddChatMessage("- {FFFACD}Ник:{51FF61} "..bans[key].name.." {fffacd} Дом {51FF61}№"..db["HOUSE"][cleanbl]["num"].." {fffacd}Парковок: {51FF61}"..db["HOUSE"][cleanbl]["park"].." {fffacd}Район - {51FF61}"..db["HOUSE"][cleanbl]["area"], 0x0ff0000)
  ...
  local ip = sampGetCurrentServerAddress() -- пїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅ пїЅпїЅпїЅпїЅ пїЅпїЅпїЅпїЅпїЅпїЅпїЅ пїЅпїЅ пїЅпїЅпїЅпїЅпїЅпїЅпїЅ пїЅпїЅ пїЅпїЅпїЅпїЅпїЅпїЅ
	...
	sampRegisterChatCommand("hui", function()
		...
		sampAddChatMessage(db["ownersOnline"][key]["owner"], -1)
		sampAddChatMessage("[HDB] [BAN]{FFFACD} Сегодня слетает дом {51FF61}№123 {FFFACD} владельца {51FF61}Mika_Skdawd.", 0x0ff0000)
	end
--  sampAddChatMessage("{FFFACD}Добавлена инфа: {51FF61}Charlie_Cooks, {FFFACD}дом {51FF61}№334. {FFFACD}На{51FF61} 22.22.2022", 0x0ff8f50)
	--sampAddChatMessage("{FFFACD}Cлетевший с продажи дом {98FB98}№"..db["signHouse"][Key]["house"].." "..db["signHouse"][Key]["park"].."{FFFACD} владельца {98fb98}"..db["signHouse"][Key]["nick"]" - {FFFACD}слетает сегодня.{98FB98}", 0x0ff8f50)
  end)
--	sampAddChatMessage("У игрока {98FB98}"..db["signHouse"][Key]["nick"].. "{ffffff} слетит дом {98fb98}№"..db["signHouse"][Key]["house"]..". {ffffff}Слет может быть {98FB98}до 05:30", -1)
  ...
  for k, v in pairs(servers) do -- пїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅ
		...
		sampRegisterChatCommand("dompoint", function()
			chksign = not chksign
			sampAddChatMessage(chksign and "[HDB] {FFFfff}Поинт на домах владельцев - активировано." or "[HDB] {FFFfff}Поинт на домах владельцев - деактивировано", 0x0ff8f50)
		...
		sampRegisterChatCommand("hdb", function()
						...
						if signe then
							for Key, keyue in ipairs(db['signHouse']) do
								if db['signHouse'][Key]['house'] == searchsigne then
									...
									saveDB()
									sampAddChatMessage("Дом №" .. searchsigne .. " был успешно удален из списка.", 0x098FB98)
								end
							end
						end
					end
	...
	sampRegisterChatCommand("signlist", function()
		sampAddChatMessage("СПИСОК ДОМОВ, СЛЕТЕВШИХ С ПРОДАЖИ:", 0x098FB98)
			for Key, keyue in ipairs(db["signHouse"]) do
			sampAddChatMessage("{ffffff}"..db['signHouse'][Key]['nick'].. " {09fb98}- {ffffff}№"..db["signHouse"][Key]["house"].." {98fb98}до "..db['signHouse'][Key]['datah'], 0x098FB98)
		end
	end)
		...
		if arg:find(".*") then
			...
			for key, val in ipairs(db['ownersOnline']) do
				if arg:lower() == db["ownersOnline"][key]["owner"]:lower() then
						sampAddChatMessage("[HDB]{FFFACD} Владелец: {51FF61}"..arg.."{FFFACD} посещал игру {51FF61}"..db["ownersOnline"][key]["time"].." "..db["ownersOnline"][key]["data"], 0x0ff8f50)
				end
			end
			sampAddChatMessage("", -1)
		end
	...
	sampRegisterChatCommand("addhouse", function()
		...
		sampAddChatMessage(db["active"] and "[HDB] {FFFACD}Добавление/обновление домов и владельцев - включено." or "[HDB] {FFFACD}Добавление/обновление домов и владельцев - выключено.", 0x0ff8f50)
	end)
		...
		if arg:find("%d+") then
			...
			for Key, keyue in ipairs(db['HOUSE']) do
				if arg == db["HOUSE"][Key]["num"] then
					sampAddChatMessage("[HDB] {FFFACD}Дом: {51FF61}№"..db["HOUSE"][Key]["num"]..", "..db["HOUSE"][Key]["park"]..".{FFFACD} Владелец: {51FF61}"..db["HOUSE"][Key]["lastowner"]..". {FFFACD}Дата: {51ff61}"..db["HOUSE"][Key]["datas"], 0x0ff8f50)
				--	for Key2, keyu2e in ipairs(keyue["owners"]) do
						local str = table.concat(db["HOUSE"][Key]["owners"], ' ')
						sampAddChatMessage("[HDB] {FFFACD}Предудущие владельцы: {51FF61}"..str.." {c0c0c0}(Щас: "..db["HOUSE"][Key]["lastowner"]..")", 0x0ff8f50)
				--	end
				end
			end
			sampAddChatMessage("", -1)
	...
	sampRegisterChatCommand("owninfo", function(arg)
		if arg:find(".*") then
			sampAddChatMessage("", -1)
			sampAddChatMessage("[HDB] {FFFACD}Игрок {51FF61}"..arg.." {FFFACD}владеет домами:", 0x0ff8f50)
			for Key, keyue in ipairs(db['HOUSE']) do
				if arg:lower() == db["HOUSE"][Key]["lastowner"]:lower() then
						sampAddChatMessage("- {51FF61}"..db["HOUSE"][Key]["num"].." "..db["HOUSE"][Key]["park"].." {FFFACD}Район: {51FF61}"..db["HOUSE"][Key]["area"]..". {FFFACD}Дата изменения: {51FF61}"..db["HOUSE"][Key]["datas"].." "..db["HOUSE"][Key]["times"], 0x0ff8f50)
				end
			end
			sampAddChatMessage("", -1)
		end
		sampAddChatMessage(os.date"%d:%m:%y", -1)
	end)
...
function imgui.OnDrawFrame()
	if mainMenu.v then
		...
		imgui.EndChild()
		imgui.SameLine()
		imgui.BeginChild("#LAST_OWNER", imgui.ImVec2(155, 25), false)
		imgui.NewInputText("##searchLASTOWNER", searchLASTOWNER, 155, u8("Владелец"), 2)
		imgui.EndChild()
		imgui.SameLine()
		imgui.Text("          ")
		imgui.SameLine()
		if db["active"] then
		if imgui.Button(fa.ICON_POWER_OFF.. u8" ВЫКЛ", imgui.ImVec2(65, 24)) then
			db["active"] = false
			sampAddChatMessage("[HDB] {FFFACD}Добавление/обновление домов и владельцев - выключено.", 0x0ff8f50)
			saveDB()
		end
	else
		if imgui.Button(fa.ICON_POWER_OFF.. u8" ВКЛ", imgui.ImVec2(65, 24)) then
		sampAddChatMessage("[HDB] {FFFACD}Добавление/обновление домов и владельцев - включено.", 0x0ff8f50)
		db["active"] = true
		saveDB()
	end
		end
		imgui.SameLine()
		if imgui.Checkbox(u8("ОПОВЕЩЕНИЯ О ДОМАХ"), tableCheckbox["signActive"]) then
			db["signActive"] = not db["signActive"]
			sampAddChatMessage(db["signActive"] and "[HDB] {FFFACD}Оповещение информации о владельцах/домов - включено." or "[HDB] {FFFACD}Оповещение информации о владельцах/домов  - выключено.", 0x0ff8f50)
			saveDB()
		end
		imgui.SameLine()
		imgui.Text("             ")
		...
		if imgui.Checkbox(u8("- ДОМА В БАНЕ##ONLY_BAN"), slot14_a2516) then
			slot15_a2515.v = false
		end
		...
		if not slot14_a2516.v and not slot15_a2515.v and searchNUM == "" and searchPARK == "" and searchAREA == "" and searchLASTOWNER == "" then
		else
			for slot8_a2460 = #db.HOUSE, 1, -1 do
				if slot14_a2516.v then
		...
		imgui.Text(u8(" База данных пустая. Включите скрипт для добавление домов."))
	end

	if #db.HOUSE > 0 and not slot14_a2516.v and not slot15_a2515.v and searchNUM == "" and searchPARK == "" and searchAREA == "" and searchAREA == "" then
		slot1_a1787.BeginChild("##BUTTON_MENU_FOOTER", slot1_a1787.ImVec2(-1, 45), true)



		slot1_a1787.EndChild()
	end
...
function imgui.GetDate(abs)
	imgui.Columns(7, "COLUMNS_CENTER", true)
	...
	imgui.NextColumn()
	imgui.SetColumnWidth(-1, 150)
	imgui.Text(u8(db.HOUSE[abs].lastowner))
	if imgui.IsItemClicked() then
	...
	if imgui.Selectable(u8'   Просмотреть ##'..abs, false) then
		sampProcessChatInput("/gethouse "..db.HOUSE[abs].num)
			imgui.OpenPopup(u8'Владельцы дома №'..db.HOUSE[abs].num.."##"..abs)
		end
	else
		imgui.Text("             -")
end
...
function ownerPlayer(ownerList)
	imgui.SetNextWindowSize(imgui.ImVec2(350, 220))

	if imgui.BeginPopupModal(u8("Владельцы дома №" .. db.HOUSE[ownerList].num .. "##" .. ownerList), true, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove) then
		imgui.BeginChild("##BEGIN_CENTER" .. ownerList, imgui.ImVec2(0, 155), true)

		for logOwn = #db.HOUSE[ownerList].owners, 1, -1 do
			local OWNS = db.HOUSE[ownerList].owners[logOwn]

			imgui.BeginChild("##BEGIN_OWNER" .. logOwn, imgui.ImVec2(230, 25), false)

			if logOwn == #db.HOUSE[ownerList].owners then
				imgui.Text(fa.ICON_CHECK_SQUARE .. " " .. logOwn .. ") " .. OWNS)
			else
				imgui.Text(fa.ICON_MINUS_SQUARE .. " " .. logOwn .. ") " .. OWNS)
			end

			imgui.EndChild()
			imgui.SameLine()
			imgui.BeginChild("##BEGIN_BUTTON" .. logOwn, imgui.ImVec2(90, 25), false)


			imgui.SameLine()

			if imgui.Button(fa.ICON_TRASH .. "##" .. logOwn, imgui.ImVec2(25, 20)) then
				table.remove(db.HOUSE[ownerList].owners, logOwn)
				saveDB()
			end
	...
	while true do wait(0)
		for i = 0, sampGetMaxPlayerId() do
			if sampIsPlayerConnected(i) then
				...
				if name == db["HOUSE"][key]["lastowner"] then
	--	wait(1000)
...
function seaTable(stbale)
	if stbale ~= nil then
		...
		if searchAREA == nil then
			...
			slot2_a2900 = searchAREA
		end

		if searchLASTOWNER == nil then
			local slot3_a2917 = ""
		else
			slot3_a2917 = searchLASTOWNER
		end
		if db["HOUSE"][stbale]["num"]:find(searchNUM.v) then
			if db.HOUSE[stbale].park:find(searchPARK.v) then
			if db.HOUSE[stbale].area:lower():find(searchAREA.v:lower()) then
				if db.HOUSE[stbale].lastowner:lower():find(searchLASTOWNER.v:lower()) then
					return true
				end
				end
			end
...
--[[	if chksign then
	...
	if db["active"] then
		if tostring(data.text):find('^%d+  %d+\n\n{ffffff}Владелец:{fbec5d} .*') then
			...
			if not oneOwn(owner) then
				table.insert(db["oneOwn"], {
				["owner"] = owner
			})
			saveDB()
		end
					...
					if num == db['HOUSE'][Key]['num'] then
						...
						db["HOUSE"][Key]["datas"] = datas
							if db["signActive"] then
								sampAddChatMessage("[HDB] {FFFFFF}Обновился владалец дома {51FF61}№"..db["HOUSE"][Key]["num"]..". {FFFFFF}Новый: {51FF61}"..owner..". {FFFFFF}Предыдущий: {51FF61}"..db['HOUSE'][Key]['lastowner'], 0x0ff8f50)
							end
							table.insert(db["HOUSE"][Key]["owners"], owner)
							db["HOUSE"][Key]["times"] = times
							--for Key, keyue in pairs(db['HOUSE']) do
								for Key2, keyu2e in ipairs(keyue["owners"]) do
									if num == db["HOUSE"][Key]["num"] then
									if #keyue["owners"] > 4 then
										table.remove(keyue["owners"], Key2)
							--			sampAddChatMessage(#keyue["owners"], -1)
										saveDB()
								--	end
									end
								end
				...
				for Key, keyue in ipairs(db['HOUSE']) do
					...
					if tostring(data.text):find("^%d+  %d+\n\n{ffffff}Владелец:{fbec5d} .*\n\n{ffffff}Владелец продает.*") then
		 			 ...
		 			 if not SellHouse(selnum) then
		 				 ...
		 				 saveDB()
		 				 if db["signActive"] then
		 				 sampAddChatMessage("{FFFACD}Дом {51FF61}№"..selnum.."{FFFACD} выставлен на продажу. Владелец: {51FF61}"..selowner..". {FFFACD}Дата: {51FF61}"..times.." "..datas, 0x0ff8f50)
		 			 end
		 			 end
		 		 else
		 			 if tostring(data.text):find("^%d+  %d+\n\n{ffffff}Владелец:{fbec5d} .*") then
						...
						local year = os.date("%y",os.time()+259200)
		 				 local noselnum, noselpark, noselowner = tostring(data.text):match("^(%d+)  (%d+)\n\n{ffffff}Владелец:{fbec5d} (.*_%a+)")
		 				 for Key, keyue in ipairs(db["sellHouse"]) do
		 					if noselnum == db["sellHouse"][Key]["selnum"] then
		 						if db["signActive"] then
									sampAddChatMessage("[HDB] {FFFFFF}Слетел с продажи дом {51FF61}№"..noselnum.." "..noselpark..".{FFFFFF} Владелец: {51FF61}"..db["sellHouse"][Key]["sellastowner"]..". {FFFFFF}Дата: {51FF61}"..db["sellHouse"][Key]["times"].." "..db["sellHouse"][Key]["datas"], 0x0ff8f50)
			 					end
						--[[	if chksign then -- // Было для ловлю слетевших табличке теперь не актцально...
							if db["sellHouse"][Key]["sellastowner"] == noselowner then
								if not ChkSign(noselnum) then
				 				 ...
				 				 saveDB()
								 	sampAddChatMessage("{FFFACD}Добавлена инфа: {51FF61}"..noselowner..", {FFFACD}дом {51FF61}№"..noselnum.." "..noselpark..". {FFFACD}На {51FF61}"..day.."."..mes.."."..year, 0x0ff8f50)
								end
							 end
						 end--]]
		...
		if text:find("^%d+  %d+\n\n{ffffff}Владелец:{fbec5d} .*\n\n{ffffff}Владелец продает.*") then
			local selnum, selpark, selowner = text:match("^(%d+)  (%d+)\n\n{ffffff}Владелец:{fbec5d} (.*_%a+)")
			 ...
			 if not SellHouse(selnum) then
				 ...
				 saveDB()
				 if db["signActive"] then
				 sampAddChatMessage("[HDB] {FFFACD}Дом {51FF61}№"..selnum.."{FFFACD} выставлен на продажу. Владелец: {51FF61}"..selowner..". {FFFACD}Дата: {51FF61}"..times.." "..datas, 0x0ff8f50)
			 end
			 end
		 else
			 if text:find("^%d+  %d+\n\n{ffffff}Владелец:{fbec5d} .*") then
	...
	if ip then
		...
		--	sampAddChatMessage("2", -1)
			...
			--		sampAddChatMessage(u8:decode(val), -1)
				...
				if getbanlist then
					sampAddChatMessage("{FF7F50}[HDB] {ffffff}Баны были успешно загружены.", 0x0ff8f50)
				end
...
function sampev.onRemove3DTextLabel(id)
	if chksign and #CheckingHouse > 0 then
...
--[[function bannedPlayer()
	...
	for cleanbl = #db["HOUSE"], 1, -1 do
		for key, value in pairs(bans) do
			...
			if bans[key].date.day == timeDay and bans[key].date.month == timeMonth and bans[key].date.year == timeYear then
			if db["HOUSE"][cleanbl]["lastowner"]:lower() == value.name:lower() and not checkUnBan(value.name, value.date) then
	sampAddChatMessage("[HDB] [BAN]{FFFACD} Сегодня слетает дом {51FF61}№"..db["HOUSE"][cleanbl]["num"].." {FFFACD} владельца {51FF61}"..bans[key].name..".", 0x0ff0000)
end
end
end
end
	...
	while true do wait(1000)
		for Key, keyue in pairs(db['signHouse']) do
			if db["signHouse"][Key]["day"] < os.date("%d") and db["signHouse"][Key]["mes"] <= os.date("%m") and db["signHouse"][Key]["year"] <= os.date("%y") and "05" < os.date("%H") then
				sampAddChatMessage("{ffffff}Дом {98FB98}№"..db["signHouse"][Key]["house"].. "{ffffff} Удален из списка инфы из-за срока давности.", 0x098FB98)
				table.remove(db["signHouse"], Key)
			end
		end
	end
...
function ChkSign(str)
 ...
 function oneOwn(str)
	 if str ~= nil then
		 for key, key in ipairs(db["oneOwn"]) do
		 if key["owner"] == str then
				 return true
		 end
		 end
	 end
...
function update()
  ...
  downloadUrlToFile('https://raw.githubusercontent.com/vitomc1/HDB/main/version.json?token=GHSAT0AAAAAAB7SQV2RCNBXO4Q4DLA5BSQIZADNGYQ', fpath, function(id, status, p1, p2) -- ссылку на ваш гитхаб где есть строчки которые я ввёл в теме или любой другой сайт
    ...
    if f then
      ...
      if info and info.latest then
        ...
        if version > tonumber(thisScript().version) then -- если версия больше чем версия установленная то...
          ...
          update = false -- не даём обновиться "{FF7F50}[HDB] {ffffff}База Данных домов игроков. /hdb - основное меню", 0x0ff8f50
          sampAddChatMessage('{FF7F50}[HDB]: {ffffff}Ваша версия скрипта актуальная. Обновление не требуется. Версия: '..thisScript().version, -1)
        end
      end
    end
  end
...
function goupdate()
sampAddChatMessage('{FF7F50}[HDB]: {ffffff}Обнаружено обновление. AutoReload может конфликтовать. Обновляюсь...', -1)
sampAddChatMessage('{FF7F50}[HDB]: {ffffff}Текущая версия: '..thisScript().version..". Новая версия: "..version, -1)
wait(300)
downloadUrlToFile(updatelink, thisScript().path, function(id3, status1, p13, p23) -- качает ваш файлик с latest version
  if status1 == dlstatus.STATUS_ENDDOWNLOADDATA then
		local _, id = sampGetPlayerIdByCharHandle(playerPed)
  sampAddChatMessage('{FF7F50}[HDB]: {ffffff}Обновление завершено!', -1)
  thisScript():reload()
end
