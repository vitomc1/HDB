	...
	--	lua_thread.create(bannedPlayer)
		while true do wait(0)
			if updMSG then
				...
				sampAddChatMessage("[HDB] {FBEC5D}Скрипт недавно обновился до "..thisScript().version.." узнать изменения - /hdb upd", 0xFF2B2B)
				updMSG = false
			end
			end
...
--[[	if dompoint then
	...
	if tostring(data.text):find("{ffffff}Это жилье продается за {33aa33}") then
		checkTEXT = false
	elseif tostring(data.text):find("Домашний замок был открыт") or tostring(data.text):find("Домашний замок был заперт") then
		checkTEXT = false
	elseif tostring(data.text):find("%d+%s %d%s%s{ffffff}Дом выставлен на аукцион") then
		checkTEXT = false
	elseif tostring(data.text):find("%d+%s %d%s%s{ffffff}Владелец:{fbec5d}") or tostring(data.text):find("%d+%s %dG%s%s{ffffff}Владелец:{fbec5d}") then
		checkTEXT = true
	end
				...
				for Key, keyue in ipairs(db['HOUSE']) do
					...
					if tostring(data.text):find("^%d+  %d+.*\n\n{ffffff}Владелец:{fbec5d} .*\n\n{ffffff}Владелец продает.*") then
		 			 ...
		 			 if not SellHouse(selnum) then
		 				 ...
		 				 saveDB()
		 				 if db["signActive"] then
		 				sampAddChatMessage("[HDB] {FFFACD}Дом {FBEC5D}№"..selnum.."{FFFACD} выставлен на продажу. Владелец: {FBEC5D}"..selowner, 0xFF2B2B)
		 			 end
				 else
					 for key, val in ipairs(db["sellHouse"]) do
		 				 ...
		 				 for Key, keyue in ipairs(db["sellHouse"]) do
							...
							if db["sellHouse"][Key]["sellastowner"] == noselowner then
								if not ChkSign(noselnum) then
				 				 ...
				 				 saveDB()
								 	sampAddChatMessage("[HDB]: {e0e094}Добавлена инфа (/saleset): {FBEC5D}"..noselowner..", {e0e094}дом {FBEC5D}№"..noselnum.." "..noselpark..". {e0e094}На "..day.."."..mes.."."..year, 0x0066FF)
								end
							 end
						 end
